...
let tagName = computed(() => {
  ...
  tagCounts.value?.map((i: ITag) => TagMap.set(i.id, i.name));
  let map = Object.fromEntries(TagMap);
  return map[props.id];
});

const onCurrentPageChanged = async (pageNum: number) => {
  const res = await getPostArticleListApi(pageNum, pageSize, undefined, props.id);
