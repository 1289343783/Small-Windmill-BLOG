 ...
 */
function linearColorWordCloud(
    words: Array<{ id: number, count: number, name: string }>,
    config: { lowColor: string, colorSteps: Array<number>, lowSize: number, sizeStep: number, maxSize: number }) {
    let defaultConfig = {
        lowColor: "#999999",
        colorSteps: [0, 1, 2],
        lowSize: 1.1,
 ...
 * @return {Array<object>} clouds 词云
 */
function randomColorWordCloud(
    words: ITag[],
    config?: { colors: Array<string>, lowSize: number, sizeStep: number, maxSize: number }) {
    let defaultConfig = {
        colors: [
