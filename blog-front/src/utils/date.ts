 ...
 * @returns time
 */
function getRunTime(dateString: string, currentTime: number) {
    const dateStrings: string[] = dateString.split('-');

    let date = currentTime ? new Date(currentTime) : new Date();
    date.setUTCFullYear(parseInt(dateStrings[0]), parseInt(dateStrings[1]) - 1, parseInt(dateStrings[2]));
